---
name: anomaly-alert-webhook
description: Parse data observability alerts (Monte Carlo freshness drops, volume anomalies) and format structured Slack/Teams incoming webhook JSON payloads for operations response.
---

# anomaly-alert-webhook

A rigid blueprint for translating data observability alerts into structured messaging platform notifications. This skill ingests alert payloads from monitoring tools (Monte Carlo, Great Expectations, custom monitors), classifies severity, and generates platform-ready Slack or Teams webhook JSON. Run this whenever Mollie needs to route a data quality alert to operations.
  ...
  "blocks": [
    {
      ...
      "text": {
        "type": "plain_text",
        "text": "🚨 {severity}: {dimension} alert on {source}"
      }
    },
    {
      "type": "section",
    ...
    },
    {
      ...
      "elements": [
        {
          "type": "button",
          "text": {"type": "plain_text", "text": "View in Monte Carlo"},
          "url": "{link}"
        }
      ]
    }
  ...
  "attachments": [
    {
      ...
      "content": {
        ...
        "actions": [
          {
            "type": "Action.OpenUrl",
            "title": "Investigate in Monte Carlo",
            "url": "{link}"
          }
        ]
      }
...
If the input contains multiple alerts (batch alert CSV or array):
...
| 2 | Every payload MUST have a link to investigate (Monte Carlo URL, runbook, or dashboard) | Hard block |
...
| 3 | Threshold value is missing (null) | Use "no threshold configured" — flag as incomplete monitoring setup |
| 4 | Link field is empty or missing | Use fallback: runbook URL from the alert source or "request investigation" |
| 5 | Alert source table name contains PII (e.g., "users", "customers") | Flag; suggest obfuscation in shared channels |
