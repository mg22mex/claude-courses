---
name: csv-analytics
description: Cross-examine two disparate data exports (e.g., ad-spend arrays and Shopify net sales), join on dates or product codes, run margin calculations, and export a profitability summary.
---

# csv-analytics

A strict operational playbook for joining two disparate CSV exports — typically a cost/spend table and a revenue/sales table — and producing a precise profitability analysis. This skill handles schema mismatches, date-alignment gaps, and multi-granularity joins (day-level vs. order-level). Run this whenever Mollie needs to reconcile ad spend against sales, calculate blended margins, or export a channel-by-channel P&L.
...
Channels often diverge between systems (Google Ads calls it "google_shopping" while Shopify calls it "shop" or "web"). Build a normalisation map:
...
Apply this map to both tables before joining. List every mapping that fired and flag any channel value that matched no rule — those need manual review.
...
spend_ratio      = (total_spend / gross_sales) * 100
                 → set to NULL if gross_sales = 0
roas             = gross_sales / total_spend
                 → set to NULL if total_spend = 0
```
...
Apply these strict flags after calculation:
...
### 6.3 Write a channel P&L summary
...
If a channel had zero spend (e.g., organic/direct), leave `roas` blank and note `no_spend` in flags.

### 6.4 Write a JSON summary
...
1. **Never join without confirming classification.** If the cost/spend vs. revenue/sales assignment is ambiguous, stop and ask Mollie. Guessing wrong produces a meaningless report.
2. **Always use FULL OUTER JOIN.** An INNER join silently drops spend-only and revenue-only rows, which are precisely the anomalies Mollie needs to see.
3. **Never modify originals.** All cleansing, mapping, and aggregation happens in-memory. Output files are always separate exports.
4. **Never divide by zero.** If `gross_sales = 0`, set `margin_pct = NULL` and `spend_ratio = NULL`. If `total_spend = 0`, set `roas = NULL`. Print clear `N/A` markers in terminal output.
5. **Always report join quality.** Print the `=== JOIN QUALITY ===` block before any margin calculations so Mollie knows how much data was mismatched before she trusts the numbers.
6. **Flag thresholds are non-negotiable.** Do not silently move or relax the 0%/10%/20% margin thresholds or the 1x/2x ROAS thresholds. If Mollie wants custom thresholds, she must explicitly ask — default to these until overridden.
...
| Date columns have different ranges | Warn if ranges don't overlap: "revenue data starts 2026-06-01 but spend data starts 2026-06-15 — days 1–14 have no spend attribution" |
| More than two files loaded | Warn and ask Mollie to confirm which two to use |
| Single row in a file after aggregation | Validate normally but warn "only 1 aggregated row in Table B — verify the file covers the full period" |
| File has no header row | Attempt to infer schema from first data row; flag as `inferred_header` and show Mollie the result for confirmation |
| Extreme ROAS outliers (>100x) | Flag and note "ROAS > 100 — possible missing spend data or data entry error" |
| Both files classify as the same type | Print "both files appear to be revenue/sales (or both cost/spend) — cannot determine which is which. Please label them." |
