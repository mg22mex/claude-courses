...
Use the inline sample ideas below. Copy them into a text file named `ideas.txt` and upload it using the paperclip icon.
...
### Step 1 — Review the write-a-prd preset

Open the Weatherman AI Portal in your browser. Select "Rick" from the sidebar dropdown. Upload the `presets/write-a-prd/SKILL.md` file using the paperclip icon.

Type this prompt into the chat input:

> "Read the uploaded SKILL.md file and summarize:
> - What inputs does the PRD preset expect?
> - What sections will it generate?
> - What rules and constraints does it enforce?"

### Step 2 — Load and inspect the business ideas

Upload the `ideas.txt` file using the paperclip icon, then type this prompt:
...
### Step 3 — Generate a PRD for one idea using the preset

Type this prompt into the chat input:

> "Run the write-a-prd preset. Here is the concept:
>
...
Repeat the preset invocation for the remaining three ideas. For each one, provide reasonable context about target users, existing systems, and constraints.

After generating all four PRDs, type this prompt:
...
> "Write a portfolio overview containing:
...
>
> Save each generated PRD as prd-idea-01.md through prd-idea-04.md."

Use the download button to save each generated document.
...
- A `prd-portfolio-summary.md` comparing all initiatives
- A prioritization recommendation based on timeline, risk, and impact
- Practical experience with the `write-a-prd` executive preset
