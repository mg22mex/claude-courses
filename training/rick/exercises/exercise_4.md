...
The executive team needs a board-ready carrier performance review pack. You have fulfillment data covering multiple carriers across several weeks. Your job is to analyse on-time rates, identify value at risk, and generate a structured board deck outline that the `ppt-generation` skill can turn into a presentation.

## Learning Objectives

- Load and analyse carrier-level fulfillment data
...
- Use the `ppt-generation` skill to produce a presentation-ready deck

## Dataset

Use `fulfillment_delay_report.csv` from the data directory.

### File: `../data/fulfillment_delay_report.csv`
...
### Step 1 — Load and inspect fulfillment data

```bash
claude ../data/fulfillment_delay_report.csv
```

Prompt:

```
Summarize this fulfillment dataset:
- How many orders
...
After the skill generates the deck, save it:

```
Write the generated board deck to board_deck_outline.md.
...
- A `board_deck_outline.md` with 8 structured slides ready for presentation building
- Practical experience using the `ppt-generation` executive skill
