---
name: warehouse-balancing
description: Parse multi-location inventory levels, identify stockout risks based on historical velocity, and output optimal regional transfer quantities.
---

# warehouse-balancing

A strict operational playbook for balancing inventory across multiple warehouse locations. This skill takes inventory snapshots from each warehouse alongside historical sales velocity data, identifies SKUs at risk of stockout at each location, and calculates the optimal transfer quantity from overstocked to understocked warehouses. Run this whenever Sunny needs to decide what to move, where, and how much — before a stockout hits.
...
### 5.2 Write the transfer plan CSV
...
### 5.3 Write the stockout risk CSV
...
### 5.4 Write the inventory summary CSV

Write `inventory_summary_<YYYYMMDD>.csv` — one row per SKU, one column per warehouse:
