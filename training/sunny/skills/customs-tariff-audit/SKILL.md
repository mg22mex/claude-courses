---
name: customs-tariff-audit
description: Parse HS codes and international freight lines, cross-reference against a tariff percentage matrix, and identify customs overpayments or misclassified goods.
---

# customs-tariff-audit

A strict operational playbook for auditing international freight invoices against customs tariff schedules. This skill takes a shipment manifest with HS codes and declared values, cross-references each line against a reference tariff matrix, and flags discrepancies where the duty applied does not match the scheduled rate. Run this whenever Sunny receives a customs invoice from a freight forwarder and needs to verify duty assessments before payment.
...
### 5.2 Write the detailed line-item CSV
...
### 5.3 Write the origin summary CSV (if origin data exists)
