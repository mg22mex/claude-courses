...
description: Systematically audit live e-commerce product listings — parse item copy, title tags, meta descriptions, and image alt tags against SEO keyword targets to catch and flag compliance errors.
---

# listing-verification

A structured playbook for auditing e-commerce product listings against SEO and brand standards. This skill parses listing copy, title tags, meta descriptions, image alt text, and pricing data to flag missing keywords, duplicate content, broken metadata, and compliance gaps. Run this whenever Christine needs to verify a batch of product listings before publishing or after a bulk update.
...
Accept a keyword target table (from `data/keyword_targets.csv` or inline):
...
Assign a grade per keyword per field:
...
- **Brand suffix**: must end with `| BrandName` (e.g., `"Wireless Headphones | CloudSync"`)
- **Duplicate titles**: flag if two SKUs share the exact same title tag
- **Pipe separator**: must have exactly one `|` separating the descriptive title from the brand
...
|---|---|---|---|---|
| WH-100 | title_tag | Too short (22 chars) | high | Expand to 30-60 chars: "Wireless Headphones | CloudSync" |
| WH-100 | image_alt_tags | alt="image" for img_03.jpg | medium | Replace with "Noise-cancelling wireless headphones on ear" |
| WH-101 | body_html | 92% duplicate of WH-100 | high | Rewrite unique feature bullets for WH-101 |
```
...
- **Fix file**: `listing_fixes.csv` — only products with violations, with recommended corrections for each
...
| 3 | Product has multiple variants (size/color) sharing a parent SKU | Check each variant's title_tag separately for uniqueness |
| 4 | Keyword target list is empty or not provided | Run technical audit only (metadata, duplicates, alt tags); skip keyword compliance |
