...
description: Draft, format, and orchestrate automated email sequences (welcome flows, cart abandonment, post-purchase notifications) using rigid template tag variables, markdown/HTML layout parameters, and brand-voice constraints.
---

# email-automation

A structured playbook for building and auditing automated email campaigns with Claude Code. This skill handles template tag validation, brand-voice enforcement, subject-line optimization, multi-step sequence orchestration, and send-log analysis. Run this whenever Christine needs to generate or inspect email automation workflows.
...
Use the detected type to apply the correct tone rules and required template tags.
...
Build a list of required merge variables based on the campaign type (see 1.2). Common tags:
...
| {{order_number}} | ⚠️ not required for welcome flow | — | — |
```
...
Apply the following brand rules to all generated or audited copy:
...
- **CTA construction** — every email must have exactly one primary CTA button/link expressed as an action verb ("Get Started", "Track Your Order", "Claim Offer")
...
Output a single `.md` or `.html` file containing:
...
Include a **Campaign Spec Sheet** with:

| Field | Value |
|---|---|
| Campaign type | Welcome / Abandonment / Post-purchase |
| Recipient segment | New signups / Cart abandoners / Recent buyers |
| Send timing | Immediate + 24h delay |
| Primary goal | Activation / Recovery / Retention |
| Success metric | Open rate ≥ 40%, CTR ≥ 8% |

### 5.2 For send log analysis

Output a **Performance Summary**:
...
| 3 | Subject line is exactly 61 characters | Flag; suggest trimming filler words ("the", "a", "your") |
| 4 | Conditional logic references undefined variables (`{% if {{missing_var}} %}`) | Hard error — undefined conditional |
| 5 | HTML template has unclosed `<table>`, `<div>`, or `<a>` tags | Flag; auto-close if unambiguous |
